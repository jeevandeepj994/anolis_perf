...
inline bool cpus_share_cache(int this_cpu, int that_cpu)
{
...
int smc_ipi_get_cpu(struct smc_sock *smc)
{
	...
	// domain, then used the prefered ipi cpu.
	if (likely(cpus_share_cache(smc->last_cpu,
	           smc->ipi_preferred_cpu)))
		return smc->ipi_preferred_cpu;

remap:
	// otherwise, update prefered ipi cpu from cpu maps,
	...
	smc->ipi_preferred_cpu =
		cpus_share_cache(smc->last_cpu, smc->ipi_preferred_cpu) ?
