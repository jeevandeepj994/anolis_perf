...
// put tx credits, when some failures occured after tx credits got
// or recieve announce credits msgs
...
{
	return link->credits_enable && atomic_read(&link->peer_rq_credits) <= link->peer_cr_watermark_low;
}
...
{
	return link->credits_enable && atomic_read(&link->local_rq_credits) >= link->local_cr_watermark_high;
}
