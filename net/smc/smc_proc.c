	...
	list_for_each_entry_safe(lgr, lg, &smc_lgr_list.list, list) {
		for (i = 0; i < SMC_LINKS_PER_LGR_MAX; i++) {
			...
			seq_printf(seq, " %-6s%-6s%-5d%-7d%-6d%-7d%-7d%-7d%-4d%-4d%-6u%-6d%-6d%-6u%-6u%-7lu\n",
				lgr->is_smcd? "D" : "R", lgr->role == SMC_CLNT? "C" : "S", i, lgr->conns_num,
				atomic_read(&lnk->conn_cnt), lnk->state, lnk->roce_qp? lnk->roce_qp->qp_num : 0,
				lnk->peer_qpn, lnk->wr_tx_cnt, lnk->wr_rx_cnt, lnk->credits_enable,
				atomic_read(&lnk->local_rq_credits), atomic_read(&lnk->peer_rq_credits),
				lnk->local_cr_watermark_high, lnk->peer_cr_watermark_low, lnk->flags);
		}
	}
