...
static int sm4_cipher_ctr(struct skcipher_request *req, struct sm4_cipher_data *cw)
{
	...
	while ((nbytes = walk.nbytes) > 0) {
		...
		while (nbytes >= SM4_BLOCK_SIZE) {
			...
			iv = rep_xcrypt_ctr(walk.src.virt.addr, walk.dst.virt.addr, ctx->rkey_enc, walk.iv, cw, blocks);
	...
	while ((blocks = (walk.nbytes / SM4_BLOCK_SIZE))) {
		while (blocks--) {
			...
			for (n = 0; n < SM4_BLOCK_SIZE; n += sizeof(size_t))
				*(size_t *)(walk.dst.virt.addr + n) = *(size_t *)(walk.iv + n) ^ *(size_t *)(walk.src.virt.addr + n);
