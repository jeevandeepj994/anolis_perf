...
static int read_msr(struct pt_regs *regs, struct ve_info *ve)
{
	...
	regs->ax = lower_32_bits(tdx_fuzz(args.r11, TDX_FUZZ_MSR_READ));
	regs->dx = upper_32_bits(tdx_fuzz(args.r11, TDX_FUZZ_MSR_READ));
	return ve_instr_len(ve);
}
...
static void notrace tdx_write_msr(unsigned int msr, u32 low, u32 high)
{
	...
	regs.dx = high;
	regs.ax = low;
	ve.exit_reason = EXIT_REASON_MSR_WRITE;
	ve.instr_len = 0;
				   ...
				   unsigned long *val)
{
	...
	gpa = (pte_pfn(*pte) << PAGE_SHIFT) + (vaddr & ~page_level_mask(level));
	if (write)
		return mmio_write(size, gpa, *val);
	else
		return mmio_read(size, gpa, val);
}
