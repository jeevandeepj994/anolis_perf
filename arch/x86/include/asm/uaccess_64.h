...
copy_user_block_data_generic(void *to, const void *from, unsigned len)
{
	 ...
	 */
	alternative_call_2(copy_user_generic_unrolled, copy_user_generic_string,
			   X86_FEATURE_REP_GOOD, copy_user_enhanced_fast_string,
			   X86_FEATURE_ERMS,
...
copy_user_generic(void *to, const void *from, unsigned len)
{
	...
	defined(CONFIG_X86_Hygon_LMC_AVX2_ON)
static __always_inline __must_check unsigned long
raw_copy_block_data_from_user(void *dst, const void __user *src,
			      unsigned long size, unsigned long pages_nr)
{
	unsigned int mini_nr_pages =
		get_nt_block_copy_from_user_mini_nr_pages();
	if (mini_nr_pages && pages_nr >= mini_nr_pages)
		return copy_user_block_data_generic(dst, (__force void *)src,
						    size);
	else
...
static __always_inline __must_check unsigned long
raw_copy_block_data_to_user(void __user *dst, const void *src,
			    unsigned long size, unsigned long pages_nr)
{
	unsigned int mini_nr_pages = get_nt_block_copy_to_user_mini_nr_pages();
	if (mini_nr_pages && pages_nr >= mini_nr_pages)
		return copy_user_block_data_generic((__force void *)dst, src,
						    size);
	else
