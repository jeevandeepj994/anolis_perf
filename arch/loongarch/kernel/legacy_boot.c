...
static int acpi_parse_madt_pch_pic_entries(void)
{
	...
	if (!count) {
		pr_err("No PCH_PIC entries present\n");
		return -ENODEV;
	} else if (count < 0) {
		pr_err("Error parsing PCH_PIC entry\n");
		return count;
	}
...
static int parse_mem(struct _extension_list_hdr *head)
{
...
static int parse_vbios(struct _extension_list_hdr *head)
{
...
static int parse_screeninfo(struct _extension_list_hdr *head)
{
...
{
	struct _extension_list_hdr *fhead = NULL;
	unsigned long index;
	...
	do {
		...
		fhead = (struct _extension_list_hdr *)fhead->next;
...
static int __init add_legacy_isa_io(struct fwnode_handle *fwnode, unsigned long isa_base)
{
	...
	ret = ioremap_page_range(vaddr, vaddr + range->size,
			range->hw_start, pgprot_device(PAGE_KERNEL));
	return ret;
}
