...
### L2-OPTIONAL
用于保持已有的，但是无法确定是否有使用意义的 kconfig。
ANCK 不关注这一层级的 config，认为它们的变更不会影响 ANCK 造成严重影响，可以被任意打开或者关闭。比如 CONFIG_CAN、CONFIG_WIRELESS 这类配置，服务器场景不关注。但出于兼容已发布 ANCK 版本的考虑，Anolis 不会轻易去批量修改这类 kconfig，只会显示将它们标记成 L2。
下游衍生版如有需要，可随意覆盖。
若下游衍生版认为该层级中某些 kconfig 对于自己的场景非常重要，可提 PR 申请将其调整至 L2。
