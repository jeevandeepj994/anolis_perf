...
function check_arch() {
    ...
    echo "* Generating config file for arch: $arch"
    DIST_ARCH=${arch} make -C ${DIST_SRCROOT}/anolis dist-defconfig > /dev/null

    echo "* Checking configs for arch: $arch"
    python3 ${SCRIPT_DIR}/anolis_kconfig_check.py check \
        --rules ${SCRIPT_DIR}/L0-MANDATORY/${arch}.config \
        --level L0-MANDATORY \
        --rules ${SCRIPT_DIR}/L1-RECOMMEND/${arch}.config \
        --level L1-RECOMMEND \
        ${DIST_OUTPUT}/kernel-ANCK-generic-${arch}.config
...
# arch sw_64 and loongarch are not available now
arch_list=("x86" "arm64")
