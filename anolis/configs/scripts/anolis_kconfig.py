...
class ConfigRule():
    ...
    def lookup_order(arch: str) -> List[str]:
        return {
            "x86": ["x86", "generic"],
            "x86-debug": ["x86-debug", "x86", "generic"],
            "arm64": ["arm64", "generic"],
            "arm64-debug": ["arm64-debug", "arm64", "generic"],
        }[arch]
...
class ConfigValue():
    ...
    def from_text(line: str) -> Type["ConfigValue"] :
        ...
        RE_CONFIG_NOT_APPEAR = r'^# (CONFIG_\w+) is not appear$'
        ...
        elif re.match(RE_CONFIG_NOT_APPEAR, line):
            obj = re.match(RE_CONFIG_NOT_APPEAR, line)
            return ConfigValue(obj.group(1), None)
        return None

    def as_string(self, dump_emtpy: bool) -> str:
        if self.value == None:
            if dump_emtpy:
                return f"# {self.name} is not appear\n"
            else:
                return ""
        if self.value == "n":
...
class Config():
    ...
    def collapse_values(self, all_archs: List[str]):
        ...
        self.__collapse_value(all_archs, ["x86", "arm64"], "generic")
        if "generic" not in self.values:
        ...
        elif self.values["generic"].is_empty():
            # Optimization for such case:
            # CONFIG_FAILSLAB does not appear in x86, arm64,
            # it only appears in x86-debug, arm64-debug configs,
            # which finally causes a generic file exists, but has its value like:
            # > # CONFIG_FAILSLAB is not appear
            # which looks ugly.
            del self.values["generic"]
    ...
    def empty_instance(name: str, level: str):
        ...
        conf.add_value("generic", ConfigValue(name, None))
        return conf

    def is_empty_instance(self) -> bool:
        return len(self.values) == 1 and "generic" in self.values and self.values["generic"].is_empty()
