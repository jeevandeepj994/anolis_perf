...
int ngbe_disable_sriov(struct ngbe_adapter *adapter)
{
	...
	if (pci_vfs_assigned(adapter->pdev)) {
		e_dev_warn("VFs will not be deallocated\n");
		return -EPERM;
	}
				  ...
				  u32 *msgbuf, u32 vf)
{
	...
	entries = min_t(u16, entries, NGBE_MAX_VF_MC_ENTRIES);
...
static int ngbe_vf_reset_msg(struct ngbe_adapter *adapter, u16 vf)
{
	...
	wr32m(hw, NGBE_TDM_POOL_TE, 1 << vf, 1 << vf);
	...
	if (!is_zero_ether_addr(vf_mac)) {
		...
		dev_warn(pci_dev_to_dev(adapter->pdev),
			 "VF %d has no MAC address assigned\n", vf);
	}
				...
				u32 *msgbuf, u16 vf)
{
	...
	if (adapter->vfinfo[vf].pf_set_mac &&
	    memcmp(adapter->vfinfo[vf].vf_mac_addresses, new_mac,
		   ETH_ALEN)) {
		e_warn(drv, "VF %d attempted to set a new MAC address failed\n", vf);
		return -1;
	}
...
int ngbe_ndo_set_vf_mac(struct net_device *netdev, int vf, u8 *mac)
{
	...
	if (retval >= 0) {
		...
		if (test_bit(__NGBE_DOWN, &adapter->state)) {
			...
			dev_warn(pci_dev_to_dev(adapter->pdev),
				 "Bring the PF device up before using the VF device.\n");
		}
	} else {
		dev_warn(pci_dev_to_dev(adapter->pdev),
