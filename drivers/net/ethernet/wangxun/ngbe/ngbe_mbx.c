...
static int ngbe_poll_for_msg(struct ngbe_hw *hw, u16 mbx_id)
{
	...
	if (countdown == 0)
		ERROR_REPORT2(hw, NGBE_ERROR_POLLING,
 ...
 **/
static int ngbe_check_for_msg_vf(struct ngbe_hw *hw, u16 __always_unused mbx_id)
{
	int err = NGBE_ERR_MBX;
 ...
 **/
static int ngbe_check_for_ack_vf(struct ngbe_hw *hw, u16 __always_unused mbx_id)
{
	int err = NGBE_ERR_MBX;
 ...
 **/
static int ngbe_check_for_rst_vf(struct ngbe_hw *hw, u16 __always_unused mbx_id)
{
	int err = NGBE_ERR_MBX;
 ...
 *  returns SUCCESS if it successfully read message from buffer
 **/
static int ngbe_read_mbx_vf(struct ngbe_hw *hw, u32 *msg, u16 size, u16 __always_unused mbx_id)
{
	int err = 0;
	u16 i;
