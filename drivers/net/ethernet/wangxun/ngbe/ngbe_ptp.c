...
static void ngbe_ptp_tx_hwtstamp_work(struct work_struct *work)
{
	struct ngbe_adapter *adapter = container_of(work, struct ngbe_adapter,
										ptp_tx_work);
	struct ngbe_hw *hw = &adapter->hw;
	bool timeout = time_is_before_jiffies(adapter->ptp_tx_start + NGBE_PTP_TX_TIMEOUT);
	u32 tsynctxctl;
...
{
	bool timeout = time_is_before_jiffies(adapter->last_overflow_check + NGBE_OVERFLOW_PERIOD);
	struct timespec64 ts;
