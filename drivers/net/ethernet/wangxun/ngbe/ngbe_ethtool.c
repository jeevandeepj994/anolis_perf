	...
	if (hw->phy.type == ngbe_phy_m88e1512_sfi ||
		...
		if (hw->phy.autoneg_advertised) {
			if (hw->phy.autoneg_advertised & NGBE_LINK_SPEED_1GB_FULL) {
				if (ethtool_link_ksettings_test_link_mode(cmd, supported,
									  1000baseKX_Full))
					ethtool_link_ksettings_add_link_mode(cmd, advertising,
									     1000baseKX_Full);
				else if (ethtool_link_ksettings_test_link_mode(cmd, supported,
									       1000baseT_Full))
					ethtool_link_ksettings_add_link_mode(cmd, advertising,
									     1000baseT_Full);
				else
...
static int ngbe_get_eeprom_len(struct net_device *netdev)
{
	...
	return adapter->hw.eeprom.word_size << 1;
}
...
bool reg_pattern_test(struct ngbe_adapter *adapter, u64 *data, int reg, u32 mask, u32 write)
{
	u32 pat, val, before;
				     ...
				     unsigned int frame_size)
{
	...
	frame_size >>= 1;
	memset(&skb->data[frame_size], 0xAA, (frame_size >> 1) - 1);
	frame_size = 1;
	memset(&skb->data[frame_size + 10], 0xBE, frame_size);
	memset(&skb->data[frame_size + 12], 0xAF, frame_size);
}
...
static int ngbe_run_loopback_test(struct ngbe_adapter *adapter)
{
	...
	for (j = 0; j <= lc; j++) {
		...
		if (good_cnt != 64) {
			ret_val = 13;
			e_dev_err("run_loopback_test: recv_cnt = %d\n", good_cnt);
			break;
		}
	}
			   ...
			   struct ethtool_test *eth_test, u64 *data)
{
	...
	e_dev_info("diag_test: start test\n");
			    ...
			    enum ethtool_phys_id_state state)
{
	...
	case ETHTOOL_ID_ACTIVE:
		if (adapter->hw.phy.type == ngbe_phy_yt8521s_sfi ||
			...
			ngbe_phy_read_reg_ext_yt(hw, 0xA00B, 0, (u16 *)&adapter->led_reg);
	...
	case ETHTOOL_ID_ON:
		if (adapter->hw.phy.type == ngbe_phy_yt8521s_sfi ||
		    adapter->hw.phy.type == ngbe_phy_internal_yt8521s_sfi) {
			ngbe_phy_write_reg_ext_yt(hw, 0xA00B, 0, adapter->led_reg | 0x140);
		} else if (adapter->hw.phy.type == ngbe_phy_m88e1512 ||
					adapter->hw.phy.type == ngbe_phy_m88e1512_sfi) {
			TCALL(hw, phy.ops.write_reg_mdi, 22, 0, 3);
	...
	case ETHTOOL_ID_OFF:
		if (adapter->hw.phy.type == ngbe_phy_yt8521s_sfi ||
		    adapter->hw.phy.type == ngbe_phy_internal_yt8521s_sfi) {
			ngbe_phy_write_reg_ext_yt(hw, 0xA00B, 0, adapter->led_reg | 0x100);
		} else if (adapter->hw.phy.type == ngbe_phy_m88e1512 ||
					adapter->hw.phy.type == ngbe_phy_m88e1512_sfi) {
			TCALL(hw, phy.ops.write_reg_mdi, 22, 0, 3);
	...
	case ETHTOOL_ID_INACTIVE:
		...
		if (adapter->hw.phy.type == ngbe_phy_yt8521s_sfi ||
		    adapter->hw.phy.type == ngbe_phy_internal_yt8521s_sfi) {
			ngbe_phy_write_reg_ext_yt(hw, 0xA00B, 0, adapter->led_reg);
		} else if (adapter->hw.phy.type == ngbe_phy_m88e1512 ||
			adapter->hw.phy.type == ngbe_phy_m88e1512_sfi) {
			TCALL(hw, phy.ops.write_reg_mdi, 22, 0, 3);
				 ...
				 struct ethtool_rxnfc *nfc)
{
	...
	if (flags2 != adapter->flags2) {
		...
		if ((flags2 & UDP_RSS_FLAGS) &&
			...
			e_warn(drv, "enabling UDP RSS: fragmented packets may out of order to stack above\n");
...
static int ngbe_set_eee(struct net_device *netdev, struct ethtool_eee *edata)
{
	...
	if (!(adapter->flags2 & NGBE_FLAG2_EEE_CAPABLE))
		return -EOPNOTSUPP;
			    ...
			    struct ethtool_ts_info *info)
{
	...
	info->tx_types = (1 << HWTSTAMP_TX_OFF | HWTSTAMP_TX_ON);
...
static int ngbe_set_flash(struct net_device *netdev, struct ethtool_flash *ef)
{
	...
	if (ef->region == 0) {
		...
		if (ngbe_mng_present(&adapter->hw))
			ret = ngbe_upgrade_flash_hostif(&adapter->hw,
							ef->region, fw->data, fw->size);
		else
			ret = -EOPNOTSUPP;
	}
