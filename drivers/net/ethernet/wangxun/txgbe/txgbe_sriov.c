				   ...
				   u32 *msgbuf, u32 vf)
{
	...
	entries = min_t(u16, entries, (u16)TXGBE_MAX_VF_MC_ENTRIES);
				 ...
				 u32 *msgbuf, u16 vf)
{
	...
	if (adapter->vfinfo[vf].pf_set_mac &&
	    memcmp(adapter->vfinfo[vf].vf_mac_addresses, new_mac,
		   ETH_ALEN)) {
		e_warn(drv,
		       "VF %d already has an administratively set MAC address\n",
		       vf);
...
int txgbe_ndo_set_vf_trust(struct net_device *netdev, int vf, bool setting)
{
	...
	netif_info(adapter, drv, netdev, "VF %u is %strusted\n", vf, setting ? "" : "not ");
