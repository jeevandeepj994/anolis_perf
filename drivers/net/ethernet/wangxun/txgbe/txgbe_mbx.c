...
int txgbe_poll_for_msg(struct txgbe_hw *hw, u16 mbx_id)
{
	...
	if (countdown == 0)
		ERROR_REPORT2(hw, TXGBE_ERROR_POLLING,
 ...
 **/
int txgbe_write_posted_mbx(struct txgbe_hw *hw, u32 *msg, u16 size, u16 mbx_id)
{
	struct txgbe_mbx_info *mbx = &hw->mbx;
	int err;
 ...
 **/
int txgbe_check_for_msg_vf(struct txgbe_hw *hw, u16 __maybe_unused mbx_id)
{
	int err = TXGBE_ERR_MBX;
 ...
 **/
int txgbe_check_for_ack_vf(struct txgbe_hw *hw, u16 __maybe_unused mbx_id)
{
	int err = TXGBE_ERR_MBX;
 ...
 **/
int txgbe_check_for_rst_vf(struct txgbe_hw *hw, u16 __maybe_unused mbx_id)
{
	int err = TXGBE_ERR_MBX;
