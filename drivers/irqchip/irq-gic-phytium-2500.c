...
static DEFINE_PER_CPU(bool, phy_has_rss);
...
static void gic_cpu_sys_reg_init(void)
{
	...
	per_cpu(phy_has_rss, cpu) = !!(gic_read_ctlr() & ICC_CTLR_EL1_RSS);

	/* Check all the CPUs have capable of sending SGIs to other CPUs */
	for_each_online_cpu(i) {
		bool have_rss = per_cpu(phy_has_rss, i) && per_cpu(phy_has_rss, cpu);
...
static int __init gic_of_init(struct device_node *node, struct device_node *parent)
{
	...
	for (skt = 1; skt < MAX_MARS3_SOC_COUNT; skt++) {
		...
		mars3_gic_dists[skt].dist_base = ioremap(
				mars3_gic_dists[skt].phys_base, mars3_gic_dists[skt].size);
	}
