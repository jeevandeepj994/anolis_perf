...
const u8 zhaoxin_sm3_zero_message_hash[SM3_DIGEST_SIZE] = {
	0x1A, 0xB2, 0x1D, 0x83, 0x55, 0xCF, 0xA1, 0x7F,
	0x8e, 0x61, 0x19, 0x48, 0x31, 0xE8, 0x1A, 0x8F,
	0x22, 0xBE, 0xC8, 0xC7, 0x28, 0xFE, 0xFB, 0x74,
	0x7E, 0xD0, 0x35, 0xEB, 0x50, 0x82, 0xAA, 0x2B
};
EXPORT_SYMBOL_GPL(zhaoxin_sm3_zero_message_hash);
...
u32 zx_gmi_capability(void)
{
	...
	if (((ebx == 0x746e6543) && (ecx == 0x736c7561) && (edx == 0x48727561)) ||
		...
		pr_warn("This is not a Zhaoxin CPU! Return!\n");
		return 0;
	}
...
static inline int zhaoxin_sm3_init(struct shash_desc *desc)
{
...
static inline int zhaoxin_sm3_base_finish(struct shash_desc *desc, u8 *out)
{
...
int zhaoxin_sm3_update(struct shash_desc *desc, const u8 *data,
		unsigned int len)
{
	if (!data || !len)
		return -EINVAL;
...
EXPORT_SYMBOL(zhaoxin_sm3_update);

static int zhaoxin_sm3_final(struct shash_desc *desc, u8 *out)
{
	if (!desc || !out)
		return -EINVAL;
	...
	return zhaoxin_sm3_base_finish(desc, out);
}

int zhaoxin_sm3_finup(struct shash_desc *desc, const u8 *data,
	unsigned int len, u8 *hash)
{
	...
	return zhaoxin_sm3_final(desc, hash);
}
EXPORT_SYMBOL(zhaoxin_sm3_finup);

static struct shash_alg zhaoxin_sm3_alg = {
	.digestsize = SM3_DIGEST_SIZE,
	.init     = zhaoxin_sm3_init,
	.update   = zhaoxin_sm3_update,
	.final    = zhaoxin_sm3_final,
	.finup    = zhaoxin_sm3_finup,
...
static int __init zhaoxin_sm3_generic_mod_init(void)
{
	if (!gmi_available()) {
		pr_warn("GMI is unavailable on this platform.");
		return -ENODEV;
	}
	return crypto_register_shash(&zhaoxin_sm3_alg);
}

static void __exit zhaoxin_sm3_generic_mod_fini(void)
{
	crypto_unregister_shash(&zhaoxin_sm3_alg);
}

module_init(zhaoxin_sm3_generic_mod_init);
module_exit(zhaoxin_sm3_generic_mod_fini);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("SM3 Secure Hash Algorithm");

MODULE_ALIAS_CRYPTO("zhaoxin-sm3");
