				   ...
				   struct iov_iter *iter)
{
	...
	struct fuse_req_stat stat;

	if (!iov_iter_count(iter))
		return 0;

	fuse_io_start(ff->fm->fc->io_counter, &stat, iter->count, READ);
...
out:
	...
	fuse_io_end(ff->fm->fc->io_counter, &stat);
				    ...
				    struct iov_iter *iter)
{
	...
	fuse_io_start(ff->fm->fc->io_counter, &stat, iter->count, WRITE);
...
ssize_t fuse_passthrough_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	if (ret) {
		/* Reset vma->vm_file so that ->mmap failure has no side effects. */
		vma->vm_file = file;
...
int fuse_passthrough_open(struct fuse_dev *fud, int fd, bool write_only)
{
	...
	passthrough = kmalloc(sizeof(struct fuse_passthrough), GFP_NOFS);
	if (!passthrough) {
		res = -ENOMEM;
		goto err_free_file;
	}
