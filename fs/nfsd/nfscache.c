...
nfsd_reply_cache_alloc(struct nfsd_net *nn)
{
...
static void
nfsd_reply_cache_free_locked(struct svc_cacherep *rp, struct nfsd_net *nn)
{
			...
			struct nfsd_net *nn)
{
	...
	nfsd_reply_cache_free_locked(rp, nn);
...
nfsd_cache_search(struct nfsd_drc_bucket *b, struct svc_rqst *rqstp,
		__wsum csum, struct nfsd_net *nn)
{
	...
	list_for_each_entry(rp, rh, c_lru) {
		++entries;
		if (nfsd_cache_match(rqstp, csum, rp, nn)) {
			ret = rp;
			break;
		}
	}
...
nfsd_cache_lookup(struct svc_rqst *rqstp)
{
	...
	found = nfsd_cache_search(b, rqstp, csum, nn);
	if (found) {
		if (likely(rp))
			nfsd_reply_cache_free_locked(rp, nn);
		rp = found;
...
nfsd_cache_update(struct svc_rqst *rqstp, int cachetype, __be32 *statp)
{
	...
	hash = nfsd_cache_hash(rp->c_xid, nn);
